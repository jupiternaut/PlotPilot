...
def _read_character_context(db_path: Path, novel_id: str, limit: int = 12) -> str:
    with _connect(db_path) as conn:
        rows = conn.execute(
            """
            SELECT name, description, verbal_tic, mental_state, public_profile
    ...
    lines: list[str] = []
    for row in rows:
        parts = [str(row["description"] or row["public_profile"] or "").strip()]
        tic = str(row["verbal_tic"] or "").strip()
        mental = str(row["mental_state"] or "").strip()
        if tic:
            parts.append(f"口癖：{tic}")
        if mental and mental.upper() != "NORMAL":
            parts.append(f"当前心理：{mental}")
