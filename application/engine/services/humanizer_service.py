...
This service performs a conservative "de-AI" rewrite pass after a chapter has
already been generated. It borrows the practical rule set from the lemondy fork:
fix dialogue first, remove essay-like connective tissue, vary rhythm, and keep
plot facts unchanged.
"""
...
class HumanizerService:
    ...
    async def humanize_chapter(
        ...
        *,
        ...
        """Rewrite a chapter while preserving story facts."""
        ...
        logger.info(
            "[%s] humanizer start chapter=%s chars=%s",
            novel_id,
            chapter_num,
            len(stripped),
        )
        ...
        logger.info(
            "[%s] humanizer done chapter=%s chars=%s->%s",
            novel_id,
            chapter_num,
            len(stripped),
            len(rewritten),
