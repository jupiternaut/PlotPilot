...
这是硬性输出格式，不是建议。正文仍然只输出故事，但故事正文里必须保留这些标记。

① 只要本章规划、导演合同、上下文或 Bible 给出了稳定实体 ID，本节拍中以下位置必须用 special token：该实体首次出场、成为动作主语/宾语、被交接/使用/隐藏/发现、地点首次进入、势力首次压迫或表态、线索首次揭露。

② 格式只能是：[[char:id|角色名]]、[[loc:id|地点名]]、[[faction:id|势力名]]、[[prop:id|道具名]]、[[skill:id|能力名]]、[[clue:id|线索名]]。竖线左侧必须是已有 ID，右侧是读者看到的自然名称。

③ 不要写成普通名字再补解释；token 本身就是名字。例如写：[[char:quhan|屈寒]]把[[prop:tianqing_chip|加密芯片]]按进掌心。不要写：屈寒把加密芯片按进掌心。

④ 没有稳定 ID 时才写自然名称；严禁为未知实体编造 ID。同一实体同一段首处标记即可，隔段再次承担动作或状态变化时再次标记。

⑤ 输出前自查：本段出现的核心角色、地点、道具若在导演合同中有 ID，却没有 [[kind:id|名称]]，必须改到有标记再输出。
...
⑦ 有 ID 的核心角色/地点/道具首次出现是否用了 special token？→ 没有就改成 [[kind:id|名称]]
