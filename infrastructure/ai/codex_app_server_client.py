...
class CodexAppServerClient:
    ...
    PlotPilot only asks the official Codex sidecar to generate text. ChatGPT
    OAuth tokens remain owned by Codex CLI; this adapter never reads cookies or
    stores tokens in PlotPilot.
    """
    ...
    async def status(self) -> CodexAccountStatus:
        ...
        try:
            payload = await self.request("account/read", {"refreshToken": False}, timeout=30.0)
        except Exception as exc:
            return CodexAccountStatus(available=True, authenticated=False, error=str(exc))
        ...
        if not isinstance(account, dict):
            return CodexAccountStatus(
                available=True,
                authenticated=False,
                requires_openai_auth=bool(payload.get("requiresOpenaiAuth")) if isinstance(payload, dict) else True,
            )
        ...
        *,
        ...
        async with self._generation_lock:
            ...
            thread_params: dict[str, Any] = {
                ...
                "environments": [],
                "ephemeral": True,
                "serviceName": "plotpilot-codex-app-server",
            }
            if model.strip():
    ...
    async def _initialize(self) -> None:
        response = await self._request_once(
            "initialize",
            {
                "clientInfo": {"name": "plotpilot", "title": "PlotPilot", "version": "0.1.0"},
